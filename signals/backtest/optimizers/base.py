...
Ported from Vibe-Trading and adapted for the signals project data format:
- ``optimize()`` takes a returns DataFrame, a positions DataFrame, and dates
- ``optimize_from_prices()`` convenience method works with {ticker: price_df}
"""
...
class BaseOptimizer(ABC):
    ...
    # ------------------------------------------------------------------
    # Public entry (matrix form, from Vibe-Trading)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Convenience: from prices_dict (signals-project native format)
    # ------------------------------------------------------------------
    ...
    def optimize_from_prices(
        ...
        """Compute optimized weights from price data.

        This is the primary interface for the signals project.
        ...
        # Build returns DataFrame for selected tickers
        close_frames = {}
        for t in selected_tickers:
            if t in prices_dict:
                close_frames[t] = prices_dict[t]["close"]
        if len(close_frames) <= 1:
            return {t: 1.0 / len(selected_tickers) for t in selected_tickers}

        prices_df = pd.DataFrame(close_frames).dropna()
        if len(prices_df) < max(self.lookback // 2, 5):
            return {t: 1.0 / len(selected_tickers) for t in selected_tickers}

        returns_df = prices_df.pct_change().dropna().tail(self.lookback)
        if len(returns_df) < max(self.lookback // 2, 5):
            return {t: 1.0 / len(selected_tickers) for t in selected_tickers}

        tickers = list(returns_df.columns)
        ctx = self._build_context(returns_df, tickers)
        if ctx is None:
            return {t: 1.0 / len(selected_tickers) for t in selected_tickers}
